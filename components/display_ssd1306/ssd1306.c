...
static esp_err_t ssd1306_write_data(ssd1306_handle_t dev, const uint8_t *const data, const uint16_t data_len)
{
    ...
    ret = device->write(device->i2c_bus, device->dev_addr, data, data_len, -1);
    return ret;
}
...
static esp_err_t ssd1306_write_cmd(ssd1306_handle_t dev, const uint8_t *const data, const uint16_t data_len)
{
    ...
    if (data_len > SSD1306_CMD_LEN) {
        ESP_LOGE("ssd1306", "cmd size is too long");
        return ESP_FAIL;
    }
    ...
    ret = device->write(device->i2c_bus, device->dev_addr, device->cmd_buf, data_len + 1, -1 /* wait forever */);
    return ret;
}
...
void ssd1306_fill_point(ssd1306_handle_t dev, uint8_t chXpos, uint8_t chYpos, uint8_t chPoint)
{
    ...
    if (chXpos >= SSD1306_WIDTH || chYpos >= SSD1306_HEIGHT) {
        ESP_LOGI("ssd1306", "out of bound: chXpos: %d, chYpos: %d", chXpos, chYpos);
        return;
    }
    chPos = 7 - chYpos / 8;
    chBx = chYpos % 8;
...
ssd1306_handle_t ssd1306_create(uint16_t dev_addr, void *i2c_bus, ssd1306_i2c_write_t write)
{
