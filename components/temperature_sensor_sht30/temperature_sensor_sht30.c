...
static esp_err_t sht30_send_command(void *i2c_dev, uint16_t cmd)
{
    uint8_t cmd_bytes[2] = {cmd >> 8, cmd & 0xFF};
    return simple_i2c_master_write_to_device(i2c_dev, 0x44, cmd_bytes, sizeof(cmd_bytes), -1);

}

static esp_err_t sht30_read_measurement(void *dev, float *temperature, float *humidity) {
    uint8_t data[6];
    esp_err_t err = simple_i2c_master_read_from_device(dev, 0x44, data, 6, -1);
...
int temperature_sensor_sht30_init(void *i2c_dev, temperature_sensor_sht30_config_t *config)
{
    ...
    esp_err_t err = sht30_send_command(i2c_dev, SOFT_RESET_CMD);
    if (err != ESP_OK) {
...
int temperature_sensor_sht30_get_celsius(void* i2c_dev, float *temperature)
{
    ...
    err = sht30_send_command(i2c_dev, MEASUREMENT_SINGLE_SHOT_L_CS_DIS_CMD);
    if (err != ESP_OK) {
        return err;
    }

    // vTaskDelay(pdMS_TO_TICKS(15)); // Wait for measurement to complete
    ulp_lp_core_delay_cycles(15);

    return sht30_read_measurement(i2c_dev, temperature, NULL);
}
